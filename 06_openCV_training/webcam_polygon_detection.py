...
cv2.createTrackbar("Upper-Sauration", "Settings", 0,255,nothing)
...
while 1:
    ...
    #We need to keep these on a variable while moving the slides
    #In this way, the mask process can be done more easily.
    ...
    us = cv2.getTrackbarPos("Upper-Sauration", "Settings")
    ...
    #removing black dots on white objects
    kernel = np.ones((5,5), np.uint8)
    mask = cv2.erode(mask, kernel) #image erosion, tingles reduced, recognition improved

    #searching contours
    contours, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    for cnt in contours:
        area = cv2.contourArea(cnt)  #calculates area for each frame
        epsilon = 0.02*cv2.arcLength(cnt,True)
        approx = cv2.approxPolyDP(cnt, epsilon, True)

        x = approx.ravel()[0] # If approx is a multidimensional array, it puts it on a single line
        y = approx.ravel()[1] # this is where the contours begin
        if area > 400:
